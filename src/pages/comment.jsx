...
//             <div className="card  shadow-xl h-50 lg:h-72 w-72 lg:w-80 bgg  border border-orange-600 border-opacity-40 h-full">
//                 <div className="card-body h-full w-full p-4 ml-2">
//                     <div className="flex flex-col justify-between mt-2">
//                         <div>
//                             <h2 className="card-title lg:text-3xl sm:text-xl font-bold lg:mt-3 text-yellow-600 fontt">{comment.title}</h2>
...
//                         </div>
//                     </div>
//                     <p className="text-sm lg:text-base lg:px-2 text-green-950 text-opacity-95 mt-2">{comment.content}</p>
//                 </div>
//             </div>
//         </div>
//     );
...
// function Comment() {
//     return (
//         <div className=' relative mb-80 lg:mt-4'>
...
function CommentCard({ comment, index, isRepeat }) {
    return (
        <div
            className="w-full sm:w-80 p-4 sm:p-6 greenn bg-yellow-50 bg-opacity-15"
            data-aos="fade-up"
            data-aos-once="true"
            data-aos-delay="100"
        ...
        >
            <div className="card shadow-xl h-50 lg:h-72 w-72 lg:w-80 bgg border border-orange-600 border-opacity-40 h-full">
                <div className="card-body h-full w-full p-4 ml-2">
                    <div className="flex flex-col justify-between mt-2">
                        <div>
                            <h2 className="card-title lg:text-3xl sm:text-xl font-bold lg:mt-3 text-yellow-600 fontt">
                                {comment.title}
                            </h2>
                            <h3 className="text-sm lg:text-lg">{comment.address}</h3>
                        </div>
                    ...
                    <p className="text-sm lg:text-base lg:px-2 text-green-950 text-opacity-95 mt-2">
                        {comment.content}
                    </p>
                </div>
            </div>
...
function Comment() {
    // Duplicate the list only once for infinite scroll illusion
    const repeatedList = [...commentList, ...commentList];
    ...
    return (
        <div className="relative mb-80 lg:mt-4">
            ...
            <div className="comment-section absolute inset-0 mx-auto px-4 sm:px-8 w-9/12 lg:w-8/12 lg:px-16 py-8 mt-6 lg:mt-12">
                ...
                <div className="relative overflow-hidden mt-12">
                    <div className="flex space-x-4 sm:space-x-9 animate-scroll">
                        {repeatedList.map((comment, index) => (
                            <CommentCard
                                key={index}
                                comment={comment}
                                index={index}
                                isRepeat={index >= commentList.length}
                            />
                        ))}
                    </div>
                </div>
